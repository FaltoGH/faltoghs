...
const char *TOKEN_TYPE_STRINGS[17] = {
    "ASSIGN",
    "DIV",
    "ERROR",
    "GT",
...
private:
    ...
    Ast *whileStmt()
    {
        ...
        eat(TOKEN_TYPE::RPAREN);
        eat(TOKEN_TYPE::LCURLY);
        Ast *ret = new Ast(AST_TYPE::WHILE_STATEMENT);
        ret->child = condAst;
        ret->child2 = stmtList();
        eat(TOKEN_TYPE::RCURLY);
        return ret;
    ...
    Ast *stmtList()
    {
        // stmt_list = { ( assign_stmt, ";" ) | if_stmt | while_stmt }
        Ast *ret = new Ast(AST_TYPE::STATEMENT_LIST);
        while (getCurrentToken().type == TOKEN_TYPE::ID || getCurrentToken().type == TOKEN_TYPE::IF || getCurrentToken().type == TOKEN_TYPE::WHILE)
        {
            if (getCurrentToken().type == TOKEN_TYPE::ID)
            {
                ret->children.push_back(assignStmt());
            }
            else if (getCurrentToken().type == TOKEN_TYPE::IF)
            {
