...
const char *TOKEN_TYPE_STRINGS[22] = {
    "ASSIGN",
    "BREAK",
    "CONTINUE",
    "DIV",
...
const char *AST_TYPE_STRINGS[17] = {
    "ASSIGN",
    "BREAK",
    "CONTINUE",
    "DIV",
...
class Lexer
{
    ...
    Token nextToken()
    {
        ...
        else if ('a' <= getCurrentChar() && getCurrentChar() <= 'z')
        {
            ...
            if (ret.stringValue == "break")
            {
                ret.type = TOKEN_TYPE::BREAK;
            }
            else if (ret.stringValue == "continue")
            {
                ret.type = TOKEN_TYPE::CONTINUE;
            }
            else if (ret.stringValue == "for")
            {
...
private:
    ...
    Ast *stmtList()
    {
        ...
        // stmt_list = { ( assign, ";" ) | ( "break", ";" ) | ( "continue", ";" ) | for_stmt | if_stmt | ( print_stmt, ";" ) | while_stmt }
        Ast *ret = new Ast(AST_TYPE::STMT_LIST);
        Token currentToken = getCurrentToken();
        TOKEN_TYPE t = currentToken.type;
        while (
            (t == TOKEN_TYPE::BREAK) ||
            (t == TOKEN_TYPE::CONTINUE) ||
            (t == TOKEN_TYPE::INT) ||
            (t == TOKEN_TYPE::ID) ||
            (t == TOKEN_TYPE::FOR) ||
            (t == TOKEN_TYPE::IF) ||
        ...
        )
        {
            ...
            {
                case TOKEN_TYPE::BREAK:
                ret->addChild(new Ast(AST_TYPE::BREAK));
                eat(TOKEN_TYPE::BREAK);
                eat(TOKEN_TYPE::SEMI);
                break;
                case TOKEN_TYPE::CONTINUE:
                ret->addChild(new Ast(AST_TYPE::CONTINUE));
